...
" Exclude media directories from command-t
set wildignore+=myth/video,myth/music,*.mp3,*.mkv,*.avi
" Ack searching
...
inoremap jj <ESC>             " remap escape key
inoremap<F1> <ESC>            " get rid of annoying F1 key
nnoremap<F1> <ESC>
vnoremap<F1> <ESC>
...
if has("gui_running")
	...
	" WINDOWS SPECIFIC OPTIONS
		...
		if filereadable($VIRTUAL_ENV . '/_vimrc')
			source $VIRTUAL_ENV/_vimrc
		endif
	elseif has("gui_gtk2")
	" LINUX SPECIFIC OPTIONS
	    " Load up virtualenv's vimrc if it exists
